...
PowerShell Scripts install SQL Server on both nodes and configure WSFC with AlwaysOn feature. Tested and verified with SQL Server 2012, however, work with later versions as well (some minor changes might be required). I was used the scripts with VMM templates, so args used in some places. Provide just your values.
